...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//printf("RETURNCODE 1 for MAXREG_WR: %d \n", return_code);
	//return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, wrdata, 0, NULL, 0);
	//printf("RETURNCODE 2 for MAXREG_WR: %d \n", return_code);
	if (return_code < 0){
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//-----------------------DEBUG hardcoded to read from REVISION register
//	BYTE reg = 18;
//	printf("register ID in hex: %x. Reading REVISION to debug. \n", reg);
//	reg *= 8;		//performing left shift by 3 bits.
//	printf("%x \n", reg);
	//--------------------------
	...
	//reading
//	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 0, NULL, 1, &read_value, 0);
//	if (return_code < 0){
//		printf("!!Error in MAXreg_rd!! Error code is %d \n", return_code);
//	}
	//printf("DEBUG! %d \n", read_value);
